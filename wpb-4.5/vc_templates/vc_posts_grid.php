...
extract( shortcode_atts( array(
	...
	'filter' => '', //grid,
	'grid_thumb_size' => 'large', // 'thumbnail'
...
$use_isotope=true;
if($filter == '' && $grid_layout_mode == 'fitRows') {
	$use_isotope=false;
}
...
$css_class = 'wpb_row wpb_teaser_grid wpb_content_element ' .
  ($use_isotope ? $this->getMainCssClass( $filter ) : 'wpb_nogrid') . // Css class as selector for isotope plugin
	...
	<div class="wpb_wrapper">
		...
		<div class="teaser_grid_container">
			...
			<ul class="categories_filter vc_col-sm-12 vc_clearfix">
				...
				<li><a href="#"
					   data-filter=".grid-cat-<?php echo esc_attr($cat->term_id) ?>"><?php echo esc_attr( $cat->name ) ?></a></li>
				<?php endforeach; ?>
			</ul>
			...
			<ul class="wpb_thumbnails wpb_thumbnails-fluid vc_clearfix"
				...
				<?php $i=1; foreach ( $posts as $post ): ?>
					...
					?>
					<li
					  class="<?php if($use_isotope) echo 'isotope-item ' ?><?php echo apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $li_span_class, 'vc_teaser_grid_li', $atts ) . $post->categories_css ?>"<?php echo  $block_style ?>>
						<div<?php if($use_isotope) echo ' class="isotope-inner"' ?>>
					...
					<?php
						if(!$use_isotope) {
							echo ($i % $grid_columns_count == 0 ? '<div class="clear"></div>' : '');
						}
						$i++;
					?>
					<?php endforeach; ?>

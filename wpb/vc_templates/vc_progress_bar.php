...
foreach ( $values as $data ) {
	...
	/***/
	if($new_line['txtcolor'] == '') {
		if(isset($atts_['titles_color']) && $atts_['titles_color'] == 'custom' && isset($atts_['titles_custom_color']) && $atts_['titles_custom_color']) {
			$new_line['txtcolor']=' style="color:'.$atts_['titles_custom_color'].'"';
		}
	}
	/***/
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<span class="vc_bar' . esc_attr( $bar_options ) . '" data-percentage-value="' . ( $percentage_value ) . '" data-value="' . $line['value'] . '"' . ($line['bgcolor'] ? ' style="background-color:'.$line['bgcolor'].'"':'') . '></span>';
	$output .= '</div>';
}

 ...
 * @license   GPL-2.0+
 *
 * @modifications v1.0 by Olevmedia: update option added, dismiss notice key verification upon plugin versions added
 */
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function thickbox() {
			...
			if ( get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice_' . $this->id, true ) != $this->get_dismiss_notice_key() ) {
				add_thickbox();
			}
		...
		public function admin_menu() {
			...
			foreach ( $this->plugins as $plugin ) {
				if ( ! is_plugin_active( $plugin['file_path'] ) || ( $plugin['version'] && version_compare($installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<') ) ) {
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) ) {
				...
				// If it's update action, delete folder firstly
				if( isset( $_GET[sanitize_key( 'tgmpa-install-mode' )] ) && $_GET[sanitize_key( 'tgmpa-install-mode' )] == 'update')  {
					global $wp_filesystem;
					
					$plugin_path=$this->_get_plugin_basename_from_slug( $plugin['slug'] );
					if(is_plugin_active( $plugin_path ))
						deactivate_plugins( $plugin_path );

					$dirname=dirname($plugin_path);
					if(!empty($dirname) && $dirname != '.')
						$wp_filesystem->delete( WP_PLUGIN_DIR . '/' . $dirname, true );
				}
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				$show_update_link   = $update_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $install_link_count, 'tgmpa' ) . '</a>' : '';
		...
		public function dismiss() {
			...
			if ( isset( $_GET['tgmpa-dismiss'] ) ) {
				update_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice_' . $this->id, $this->get_dismiss_notice_key() );
			}
		...
		/**
		 * Gnerate unique dismiss key
		 */		
		protected function get_dismiss_notice_key() {
			$key=1;
			if($this->plugins) {
				$key='';
				foreach($this->plugins as $plugin) {
					$key.=$plugin['slug'].$plugin['version'];
				}
				$key=md5($key);
			}
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( $this->tgmpa->plugins as $plugin ) {
				if ( ( is_plugin_active( $plugin['file_path'] ) || ( isset( $plugin['is_callable'] ) && is_callable( $plugin['is_callable'] ) ) ) && ( empty($plugin['version']) || version_compare($installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '>=') ) ) {
				...
				elseif( $plugin['version'] && version_compare($installed_plugins[$plugin['file_path']]['Version'], $plugin['version'], '<') ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Update required', 'tgmpa' ) );
				}
				elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
		...
		public function column_plugin( $item ) {
			...
			/** Display the 'Update' or 'Activate' hover link */
			else {
				
				$actions = array();
				
				$version=$this->_get_plugin_data_from_name($item['sanitized_plugin'], 'version');
				
				if($version && version_compare($installed_plugins[$item['file_path']]['Version'], $version, '<')) {

					$update_nonce_url = wp_nonce_url(
						add_query_arg(
							array(
								'page'          => urlencode( $this->tgmpa->menu ),
								'plugin'        => urlencode( $item['slug'] ),
					...
					$actions['update'] = sprintf(
						'<a href="%1$s">' . esc_attr_x( 'Update %2$s', '%2$s = plugin name in screen reader markup', 'tgmpa' ) . '</a>',
						esc_url( $update_nonce_url ),
						'<span class="screen-reader-text">' . $item['sanitized_plugin'] . '</span>'
					);
				}
				
				if(is_plugin_inactive( $item['file_path'] )) {
